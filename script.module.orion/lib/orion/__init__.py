...
class Orion:
	...
	# Settings Category
	SettingsGeneral = OrionSettings.CategoryGeneral					# The General settings tab.
	SettingsAccount = OrionSettings.CategoryAccount					# The Account settings tab.
	SettingsFilters = OrionSettings.CategoryFilters					# The Filters settings tab.
	...
	# Retrieve the metadata of the last API call.
	@classmethod
	def last(self, encoding = None):
		return self._encode(OrionApi.last(), encoding = encoding)

	@classmethod
	def lastStatus(self):
		return OrionApi.lastStatus()

	@classmethod
	def lastStatusSuccess(self):
		return OrionApi.lastStatusSuccess()

	@classmethod
	def lastStatusError(self):
		return OrionApi.lastStatusError()
	...
	def lastTypeSubscription(self):
		return OrionApi.lastTypeSubscription()

	@classmethod
	def lastDescription(self):
		return OrionApi.lastDescription()

	@classmethod
	def lastMessage(self):
		return OrionApi.lastMessage()
	...
	# Show the addon settings dialog.
	def settingsLaunch(self, category = None, section = None, addon = None, app = None, wait = False):
		OrionSettings.launch(category = category, section = section, addon = addon, app = self.mApp if app is True else app, wait = wait)

	# Show the addon settings dialog with the filters.
	def settingsFilters(self, app = None, wait = False):
		OrionSettings.launchFilters(app = self.mApp if app is True else app, wait = wait)

	# Determine if the app uses global filters or custom filters.
	def settingsFiltersGlobal(self, app = None):
		return OrionSettings.getFiltersGlobal(app = app if app else self.mApp)
	...
	# Show a dialog with the user authentication.
	def userLogin(self):
		return OrionNavigator.dialogLogin()

	# Show a dialog with for free user registration.
	def userRegister(self):
		return OrionNavigator.dialogFree()
	...
	# Retrieve the user usage data.
	def requests(self, update = False, encoding = None):
		return self._encode(OrionUser.instance().requests(update = update), encoding = encoding)

	def requestsCount(self, update = False):
		return OrionUser.instance().requestsCount(update = update)

	def requestsStreamsTotal(self, update = False):
		return OrionUser.instance().requestsStreamsTotal(update = update)

	def requestsStreamsDailyLimit(self, update = False):
		return OrionUser.instance().requestsStreamsDailyLimit(update = update)

	def requestsStreamsDailyUsed(self, update = False, percent = False):
		return OrionUser.instance().requestsStreamsDailyUsed(update = update, percent = percent)

	def requestsStreamsDailyRemaining(self, update = False, percent = False):
		return OrionUser.instance().requestsStreamsDailyRemaining(update = update, percent = percent)

	def requestsHashesTotal(self, update = False):
		return OrionUser.instance().requestsHashesTotal(update = update)

	def requestsHashesDailyLimit(self, update = False):
		return OrionUser.instance().requestsHashesDailyLimit(update = update)

	def requestsHashesDailyUsed(self, update = False, percent = False):
		return OrionUser.instance().requestsHashesDailyUsed(update = update, percent = percent)

	def requestsHashesDailyRemaining(self, update = False, percent = False):
		return OrionUser.instance().requestsHashesDailyRemaining(update = update, percent = percent)

	def requestsContainersTotal(self, update = False):
		return OrionUser.instance().requestsContainersTotal(update = update)

	def requestsContainersDailyLimit(self, update = False):
		return OrionUser.instance().requestsContainersDailyLimit(update = update)

	def requestsContainersDailyUsed(self, update = False, percent = False):
		return OrionUser.instance().requestsContainersDailyUsed(update = update, percent = percent)

	def requestsContainersDailyRemaining(self, update = False, percent = False):
		return OrionUser.instance().requestsContainersDailyRemaining(update = update, percent = percent)
	...
	def streams(self,
		...
		if not details and result: result = result.streams()
